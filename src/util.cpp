...
const char * const BITCOIN_CONF_FILENAME = "cup.conf";
const char * const BITCOIN_PID_FILENAME = "cupd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "cup" is a composite category enabling all Cup-related debug output
            if(ptrCategory->count(string("cup"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CupCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CupCore
    // Mac: ~/Library/Application Support/CupCore
    // Unix: ~/.cupcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CupCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CupCore";
#else
    // Unix
    return pathRet / ".cupcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty cup.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override cup.conf

...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Cup related functionality
...
/*
    ...
    allow endless transaction that would bloat Cup and make it unusable. To
...
{
    if(fLiteMode) return; // disable all Cup specific functionality

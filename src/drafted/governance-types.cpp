...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "CupNetwork") type = CupNetwork;
    if(strType == "CupNetworkVariable") type = CupNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == CupNetwork) s = "CupNetwork";
    if(type == CupNetworkVariable) s = "CupNetworkVariable";
